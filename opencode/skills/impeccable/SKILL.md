...
This skill is installed via the dotfiles submodule at `_submodules/impeccable`. When running scripts, resolve the scripts directory with this priority:

1. **Project-local**: `.opencode/skills/impeccable/scripts/` in the current project (if `impeccable skills install` was run there)
2. **Environment override**: `$IMPECCABLE_SCRIPTS_DIR` (absolute path)
3. **Dotfiles submodule**: Walk up from `$HOME` looking for `_submodules/impeccable/.opencode/skills/impeccable/scripts/`, or use the resolved symlink path of this SKILL.md's parent directory
4. **npx fallback**: `npx impeccable <subcommand>` for CLI-based operations

When the SKILL.md references `node .opencode/skills/impeccable/scripts/<script>`, first try the project-local path. If it fails (file not found), fall back to the dotfiles submodule path. The `reference/` and `scripts/` directories in this skill directory are symlinks to the submodule.

### Quick discovery command

To find the scripts directory at runtime:

```bash
IMPECCABLE_SCRIPTS_DIR="${IMPECCABLE_SCRIPTS_DIR:-$(cd "$(dirname "$0")/scripts" 2>/dev/null && pwd)}" node "${IMPECCABLE_SCRIPTS_DIR}/load-context.mjs"
```

Or simply try project-local first, then the symlink-resolved path:

```bash
SKILL_DIR="$(cd "$(dirname "$(readlink -f "${BASH_SOURCE[0]:-$0}")")" && pwd)"
node "${SKILL_DIR}/scripts/load-context.mjs" 2>/dev/null || node ".opencode/skills/impeccable/scripts/load-context.mjs"
```
...
|---|---|---|
| Context | The PRODUCT.md / DESIGN.md loader result is known from running `load-context.mjs`. | Run the loader before continuing. |
...
Load both in one call (try project-local first, then dotfiles submodule):

```bash
node .opencode/skills/impeccable/scripts/load-context.mjs 2>/dev/null || node <skill-dir>/scripts/load-context.mjs
```

Where `<skill-dir>` is the resolved path of this skill's directory (follow symlinks).
...
Or if not installed locally, use the dotfiles-resolved path:

```bash
SKILL_DIR="$(cd "$(dirname "$(readlink -f "$0")")" && pwd)" && node "${SKILL_DIR}/scripts/pin.mjs" <pin|unpin> <command>
```
